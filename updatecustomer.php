...
?>
    ...
    <html>
    <head>
        <title>Update Customer:Relief Estates</title>
...
if(isset($_POST['newpassword'])&&isset($_POST['oldpassword'])&&isset($_POST['confirmpassword']))
{
    if (!empty($_POST['newpassword']) &&isset($_POST['oldpassword'])&& !empty($_POST['confirmpassword']) )
    {
        ...
        $query="UPDATE customers SET cpassword='$newpassword' WHERE cusername='$sessionname' ";
        $query2="SELECT cpassword from customers WHERE cusername='$sessionname'";
        if($query_run2 = mysql_query($query2))
        {

            $dbpassword = mysql_result($query_run2,0,'cpassword');
            $dbpassword;
