...
<html>
<head>
    <title>Customer Details:Relief Estates</title>
...
<div class="sellerdetialsdiv">
    <table class="sellerdetailstable">
        <thead>
        <tr>
            <th colspan="5"><h1>Customer Details</h1></th>
        </tr>
        <tr>
            <th>Name</th>
            <th>Username</th>
        ...
        <tbody>
        <?php
        $query="SELECT cname,cusername,cemail,ccity,ccontact FROM customers";
        if($query_run=mysql_query($query))
        {
            ...
            else
            {
                ...
                {
                    echo "<tr><td>{$row['cname']}</td><td>{$row['cusername']}</td><td>{$row['cemail']}</td><td>{$row['ccity']}</td><td>{$row['ccontact']}</td></tr>\n";
                }
            }
        }
        ?>
